...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
    <!-- Total Bookings -->
    <div class="bg-white dark:bg-[#111111] rounded-2xl shadow-sm border border-blue-100 dark:border-yellow-900/50 p-6 flex items-center transition-transform hover:scale-[1.02]">
        <div class="p-4 rounded-full bg-blue-50 dark:bg-zinc-800 text-blue-950 dark:text-yellow-400 mr-5">
        ...
        <div>
            ...
            <p class="text-3xl font-extrabold text-blue-950 dark:text-yellow-400">{{ $stats['total_bookings'] }}</p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-[#111111] rounded-2xl shadow-sm border border-blue-100 dark:border-yellow-900/50 p-6 flex items-center transition-transform hover:scale-[1.02]">
        ...
        <div>
            ...
            <p class="text-3xl font-extrabold text-blue-950 dark:text-yellow-400">{{ $stats['active_bookings'] }}</p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-[#111111] rounded-2xl shadow-sm border border-blue-100 dark:border-yellow-900/50 p-6 flex items-center transition-transform hover:scale-[1.02]">
        ...
        <div>
            ...
            <p class="text-3xl font-extrabold text-blue-950 dark:text-yellow-400">₹{{ number_format($stats['total_spent'], 0) }}</p>
        </div>
    </div>
</div>

<div class="bg-white dark:bg-[#111111] rounded-2xl shadow-sm border border-blue-100 dark:border-yellow-900/50 overflow-hidden">
    <div class="px-8 py-6 border-b border-blue-100 dark:border-yellow-900/50 flex justify-between items-center bg-white/30 dark:bg-[#111111]/50">
        <h3 class="text-xl font-bold text-blue-950 dark:text-yellow-400">Booking History</h3>
        <a href="{{ route('vehicles.index') }}" class="inline-flex items-center px-5 py-2.5 bg-[#111111] dark:bg-white text-white dark:text-blue-950 rounded-full font-bold text-xs uppercase tracking-widest hover:opacity-80 transition-opacity">
            Book New Vehicle
        </a>
    </div>
    ...
    @if($bookings->isEmpty())
        <div class="p-12 text-center">
            ...
            <h3 class="mt-2 text-sm font-bold text-blue-950 dark:text-yellow-400 uppercase tracking-wider">No bookings yet</h3>
    ...
    @else
        <div class="overflow-x-auto">
            <table class="w-full text-left whitespace-nowrap">
                <thead class="bg-white dark:bg-[#111111] border-b border-blue-100 dark:border-yellow-900/50">
                    <tr>
                ...
                <tbody class="divide-y divide-gray-50 dark:divide-zinc-800">
                    @foreach($bookings as $booking)
                        <tr class="hover:bg-white/50 dark:hover:bg-zinc-800/50 transition-colors">
                            <td class="px-8 py-5 text-sm font-bold text-blue-950 dark:text-yellow-400">#{{ str_pad($booking->id, 5, '0', STR_PAD_LEFT) }}</td>
                            <td class="px-8 py-5">
                                <div class="text-sm font-bold text-blue-950 dark:text-yellow-400">{{ $booking->vehicle->name }}</div>
                            ...
                            <td class="px-8 py-5 text-sm font-medium text-blue-800 dark:text-yellow-500">
                            ...
                            <td class="px-8 py-5">
                                <div class="text-sm font-extrabold text-blue-950 dark:text-yellow-400">₹{{ number_format($booking->total_amount, 0) }}</div>
                            ...
                            <td class="px-8 py-5">
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest 
                                    ...
                                    {{ $booking->status === 'approved' ? 'bg-[#111111] dark:bg-white text-white dark:text-blue-950' : '' }}
                                    ...
                                    {{ $booking->status === 'cancelled' ? 'bg-blue-50 dark:bg-[#111111] text-blue-600 dark:text-blue-500' : '' }}
                                ">
                                    {{ $booking->status }}
                                </span>
                            </td>
                            <td class="px-8 py-5">
                                @if($booking->vehicle->shop && $booking->vehicle->shop->payment_instructions)
                                    <div class="text-xs text-blue-800 dark:text-yellow-500 whitespace-normal max-w-xs p-3 bg-white dark:bg-zinc-800/50 rounded-lg border border-blue-100 dark:border-zinc-700 font-medium">
                                        <strong class="block mb-1 text-blue-950 dark:text-yellow-400 text-[10px] uppercase tracking-widest">Payment Instructions:</strong>
        ...
        @if($bookings->hasPages())
            <div class="px-8 py-5 border-t border-blue-100 dark:border-yellow-900/50 bg-white/30 dark:bg-[#111111]/30">
                {{ $bookings->links() }}
            </div>
        @endif
    @endif
