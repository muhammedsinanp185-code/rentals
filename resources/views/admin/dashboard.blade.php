...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
    <!-- Total Vehicles -->
    <div class="bg-white dark:bg-[#111111] rounded-xl shadow-sm border border-blue-100 dark:border-yellow-900/50 p-6 flex items-center">
        ...
        <div>
            ...
            <p class="text-3xl font-bold text-blue-950 dark:text-yellow-400">{{ $stats['total_vehicles'] }}</p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-[#111111] rounded-xl shadow-sm border border-blue-100 dark:border-yellow-900/50 p-6 flex items-center">
        ...
        <div>
            ...
            <p class="text-3xl font-bold text-blue-950 dark:text-yellow-400">{{ $stats['available_vehicles'] }}</p>
        </div>
    </div>
    
    <!-- Pending Bookings -->
    <div class="bg-white dark:bg-[#111111] rounded-xl shadow-sm border border-blue-100 dark:border-yellow-900/50 p-6 flex items-center">
        <div class="p-3 rounded-full bg-yellow-100 dark:bg-yellow-900/50 text-yellow-600 dark:text-yellow-400 mr-4">
            <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
        </div>
        <div>
            <p class="text-sm font-medium text-blue-600 dark:text-blue-500">Pending Bookings</p>
            <p class="text-3xl font-bold text-blue-950 dark:text-yellow-400">{{ $stats['pending_bookings'] }}</p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-[#111111] rounded-xl shadow-sm border border-blue-100 dark:border-yellow-900/50 p-6 flex items-center">
        ...
        <div>
            ...
            <p class="text-3xl font-bold text-blue-950 dark:text-yellow-400">{{ $stats['total_customers'] }}</p>
        </div>
    </div>
</div>

<!-- Recent Bookings Table -->
<div class="bg-white dark:bg-[#111111] rounded-xl shadow-sm border border-blue-100 dark:border-yellow-900/50 overflow-hidden">
    <div class="px-6 py-5 border-b border-blue-100 dark:border-yellow-900/50 flex justify-between items-center bg-white/50 dark:bg-[#111111]/50">
        <h3 class="text-lg font-bold text-blue-900 dark:text-yellow-400">Recent Bookings</h3>
    ...
    @if($stats['recent_bookings']->isEmpty())
        <div class="p-8 text-center">
            ...
            <h3 class="mt-2 text-sm font-medium text-blue-950 dark:text-yellow-400">No bookings</h3>
    ...
    @else
        <div class="overflow-x-auto">
            <table class="w-full text-left whitespace-nowrap">
                <thead>
                    <tr class="bg-white dark:bg-[#111111] border-b border-blue-100 dark:border-yellow-900/50 text-xs font-semibold text-blue-600 dark:text-blue-500 uppercase tracking-wider">
                ...
                <tbody class="divide-y divide-gray-50 dark:divide-gray-700">
                    @foreach($stats['recent_bookings'] as $booking)
                        <tr class="hover:bg-white/50 dark:hover:bg-gray-700/50 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <div class="text-sm font-medium text-blue-950 dark:text-yellow-400">{{ $booking->user->name }}</div>
                                <div class="text-xs text-blue-600 dark:text-blue-500">{{ $booking->user->email }}</div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-sm font-medium text-blue-950 dark:text-yellow-400">{{ $booking->vehicle->name }}</div>
                                <div class="text-xs text-blue-600 dark:text-blue-500">{{ $booking->vehicle->registration_number }}</div>
                            </td>
                            <td class="px-6 py-4 text-sm text-blue-800 dark:text-yellow-500">
