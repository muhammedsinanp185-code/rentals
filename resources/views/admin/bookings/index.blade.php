...
<div class="mb-4 flex justify-between items-center">
    <h3 class="text-lg font-bold dark:text-yellow-400">Manage Bookings</h3>
</div>
        ...
        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            @forelse ($bookings as $booking)
                <tr class="hover:bg-white dark:hover:bg-gray-700/50">
                    <td class="px-6 py-4 dark:text-yellow-500">#{{ $booking->id }}</td>
                    <td class="px-6 py-4">
                        <div class="font-medium text-blue-950 dark:text-yellow-400">{{ $booking->user->name }}</div>
                        <div class="text-xs text-blue-600 dark:text-blue-500">{{ $booking->user->email }}</div>
                    </td>
                    <td class="px-6 py-4">
                        <div class="font-medium text-blue-950 dark:text-yellow-400">{{ $booking->vehicle->name }}</div>
                        <div class="text-xs text-blue-600 dark:text-blue-500">{{ $booking->vehicle->registration_number }}</div>
                    </td>
                    <td class="px-6 py-4">
                        <div class="text-sm text-blue-950 dark:text-yellow-400">{{ $booking->start_date->format('d M Y') }}</div>
                        <div class="text-xs text-blue-600 dark:text-blue-500">to {{ $booking->end_date->format('d M Y') }} ({{ $booking->total_days }}d)</div>
                    </td>
                    <td class="px-6 py-4 font-bold dark:text-yellow-400">₹{{ number_format($booking->total_amount, 0) }}</td>
                    <td class="px-6 py-4">
                    ...
                    <td class="px-6 py-4 text-right">
                        <form action="{{ route('admin.bookings.update-status', $booking) }}" method="POST" class="flex justify-end items-center gap-2">
                            @csrf
                            @method('PATCH')
                            <select name="status" class="text-sm rounded border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-yellow-400 py-1 pr-8" onchange="this.form.submit()">
