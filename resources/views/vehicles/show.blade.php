...
@section('content')
<div class="bg-blue-50 dark:bg-black py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white dark:bg-[#111111] rounded-xl shadow-sm border border-blue-200 dark:border-yellow-900/50 overflow-hidden">
            <div class="flex flex-col md:flex-row">
                ...
                <div class="md:w-1/2 p-8 flex flex-col">
                    <div class="mb-2 flex justify-between items-start">
                        <div>
                            <h1 class="text-3xl font-bold text-blue-950 dark:text-yellow-400">{{ $vehicle->name }}</h1>
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-8">
                        <div class="bg-white dark:bg-gray-700/50 p-4 rounded-lg border border-blue-100 dark:border-yellow-900/50">
                            <span class="block text-sm text-blue-600 dark:text-blue-500 font-medium">Seats</span>
                            <span class="block text-lg font-bold text-blue-950 dark:text-yellow-400">{{ $vehicle->seats }}</span>
                        </div>
                        <div class="bg-white dark:bg-gray-700/50 p-4 rounded-lg border border-blue-100 dark:border-yellow-900/50">
                            <span class="block text-sm text-blue-600 dark:text-blue-500 font-medium">Fuel Type</span>
                            <span class="block text-lg font-bold text-blue-950 dark:text-yellow-400">{{ $vehicle->fuel_type }}</span>
                        </div>
                        <div class="bg-white dark:bg-gray-700/50 p-4 rounded-lg border border-blue-100 dark:border-yellow-900/50">
                            <span class="block text-sm text-blue-600 dark:text-blue-500 font-medium">Transmission</span>
                            <span class="block text-lg font-bold text-blue-950 dark:text-yellow-400">{{ $vehicle->transmission }}</span>
                        </div>
                        <div class="bg-white dark:bg-gray-700/50 p-4 rounded-lg border border-blue-100 dark:border-yellow-900/50">
                            <span class="block text-sm text-blue-600 dark:text-blue-500 font-medium">Status</span>
                            <span class="block text-lg font-bold text-blue-950 dark:text-yellow-400 capitalize">{{ $vehicle->status }}</span>
                        </div>
                    </div>

                    <div class="mb-8">
                        <h3 class="text-lg font-bold text-blue-950 dark:text-yellow-400 mb-2">Description</h3>
                    ...
                    <div class="mt-auto">
                        ...
                        @else
                            <div class="bg-indigo-50 dark:bg-indigo-900/30 p-6 rounded-lg border border-indigo-100 dark:border-indigo-800">
                                <h3 class="text-lg font-bold text-blue-950 dark:text-yellow-400 mb-4">Request Booking</h3>
                                @auth
                                    @if(Auth::user()->role === 'customer')
                                        <form action="{{ route('bookings.store') }}" method="POST">
                                            ...
                                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                                                <div>
                                                    <label class="block text-sm font-medium text-gray-700 dark:text-yellow-500 mb-1">Start Date</label>
                                                    <input type="date" name="start_date" id="start_date" class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-[#111111] dark:text-yellow-400 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required min="{{ date('Y-m-d') }}">
                                                </div>
                                                <div>
                                                    <label class="block text-sm font-medium text-gray-700 dark:text-yellow-500 mb-1">End Date</label>
                                                    <input type="date" name="end_date" id="end_date" class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-[#111111] dark:text-yellow-400 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required min="{{ date('Y-m-d') }}">
                                                </div>
                                            </div>
                                            
                                            <div class="mb-4">
                                                <div class="flex justify-between items-center text-sm font-medium text-gray-700 dark:text-yellow-500 mb-1">
                                    ...
                                    @else
                                        <div class="text-center p-3 bg-white dark:bg-[#111111] rounded border border-blue-200 dark:border-yellow-900/50 text-blue-800 dark:text-blue-500">
                                            Admins cannot book vehicles.
                                        </div>
                                    @endif
                                @else
                                    <div class="text-center p-4 bg-white dark:bg-[#111111] rounded-lg border border-blue-200 dark:border-yellow-900/50">
                                        ...
                                        <a href="{{ route('register') }}" class="inline-block bg-white dark:bg-[#111111] text-indigo-600 dark:text-indigo-400 border border-indigo-600 dark:border-indigo-400 py-2 px-6 rounded font-medium hover:bg-indigo-50 dark:hover:bg-gray-700 ml-2">Register</a>
                                    </div>
                                @endauth
                            </div>
                        @endif
